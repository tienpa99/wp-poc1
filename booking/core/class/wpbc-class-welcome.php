...
class WPBC_Welcome {
	...
	function section_1_0_example(){
			...
			$this->expand_section_start( $section_param_arr );
					...
					?><div class="feature-section  one-col" style="padding:0;margin-bottom:-30px;">
						<div class="col col-1" style="flex: 1 1 100%;width: 100%;">
			                <?php echo '<h4>' .wpbc_recheck_strong_symbols( 'New modern design of booking admin UI' ) . '</h4>' ; ?>
						</div>
					</div><?php
		...
		. '<li>' . wpbc_recheck_strong_symbols( '**New modern design** in the booking **admin panel** that offers a sleek interface for managing your bookings.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Refreshed** and clear look of **UI elements** with updated booking toolbars, headers, and icons, that provides enhanced usability.' ) . '</li>'
							. '</ul>';
							?>
						</div>
						<div class="col col-2 last-feature" style="flex: 1 1 100%;width: 100%;">
							<?php  echo
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**Support**. The ability to select the previous "Legacy Theme" of the booking admin panel on the Settings General page.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'Updated styles of season filters on the Resources > Filters page (available in paid versions).' ) . '</li>'
							. '</ul>';
							?>
						</div>
					</div><?php
					...
					?><div class="feature-section  one-col">
						<div class="col col-1" style="flex: 1 1 100%;width: 100%;">
							...
							<img src="<?php echo $this->asset_path; ?>9.6/wp_booking_calendar__availability_page_modern_ui_02.png"
					...
					?><div class="feature-section  two-col">
						<div class="col col-1" style="flex: 1 1 50%;width: 100%;">
							...
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**Under hood**. The CSS for the time selector has been moved from wpbc_time-selector.css to client.css. This enables the use of the \'2 columns with times\' form template even when the times picker option is not activated.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Fix** the issue of a duplicate database entry (DB:Duplicate entry 13-2023-07-19 00:00:00 for key booking_id_dates::INSERT INTO ...) that occurred when the option "Use time selections as recurrent time slots" was activated but no time slots were specified in the booking form.' ) . '</li>'
													. '</ul>' ;
							?>
						</div>
						<div class="col col-2 last-feature" style="flex: 1 1 auto;width: 60%;">
						<?php  echo
							'<h4>' .wpbc_recheck_strong_symbols( '&nbsp;' ) . '</h4>' .
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**Fix** issue where partially booked dates on the Booking > Availability page were not displaying correctly when the \'Do not change background color for partially booked days\' option was activated. However, to correctly use this feature, you must open the Booking > Availability page and click the \'Reset selected options to default values\' button in the \'User options menu\' in the top right toolbar.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Fix** the issue where the message "The date or time may be booked, or already in the past!" was incorrectly displayed when using "_hints" shortcodes for the time fields, such as [durationtime_hint].' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'Lots of other improvements and fixes...' ) . '</li>'
					...
					?><div class="feature-section  one-col">
						<div class="col col-1" style="flex: 1 1 100%;width: 100%;">
								...
								<img src="<?php echo $this->asset_path; ?>9.6/wp_booking_calendar__bookings_page_modern_ui_01.png"
			...
			$this->expand_section_start( $section_param_arr );
					...
					?><div class="feature-section  one-col" style="margin-bottom:-30px">
						<div class="col col-1" style="flex: 1 1 100%;width: 100%;">
							...
							<?php
								echo '<h4>' .wpbc_recheck_strong_symbols( 'Super easy set available / unavailable dates in calendar' ) . '</h4>' ;
								echo '<h4>' .wpbc_recheck_strong_symbols( '... just with 3 mouse clicks' ) . '</h4>' ;
								// echo '<h4>' .wpbc_recheck_strong_symbols( 'Super easy set dates availability in calendar with just three mouse clicks' ) . '</h4>' ;
							?>
							</div>

							<img src="<?php echo $this->asset_path; ?>9.5/wp_booking_calendar__availability_page_10.png"
		...
		. '<li class="big_numbers">' . wpbc_recheck_strong_symbols( '**Select** your **days** in calendar by  clicking on first and last day.' ) . '</li>'
		. '<li class="big_numbers">' . wpbc_recheck_strong_symbols( 'Choose **available** / **unavailable** status in "availability section".' ) . '</li>'
		. '<li class="big_numbers">' . wpbc_recheck_strong_symbols( 'Click on **Apply** button. That\'s it.' ) . '</li>'
							. '</ul>';
							?>
						</div>
					</div><?php
					...
					?><div class="feature-section  two-col">
						<div class="col col-1" style="flex: 1 1 50%;width: 100%;">
							...
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**Simple** configuration  of **dates availability** with 3 mouse clicks at new new **Availability** menu page.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Modern** and intuitive **interface with instant responses** on the same page.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'Reset the availability for a specific calendar by clicking on the "**Reset availability**" button at the "User Options" toolbar.  This will remove all unavailable dates from the calendar.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'New calendar skins that show **unavailable dates with stripes**. ' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'The new **"Resource unavailable days" legend item** provides distinction between old unavailable dates and the new interface at Booking > Availability page.' ) . '</li>'
							. '</ul>' ;
						...
						<div class="col col-2 last-feature" style="flex: 1 1 auto;width: 70%;">
							...
							<img src="<?php echo $this->asset_path; ?>9.5/wp_booking_calendar__availability_page_14.png"
					...
					?><div class="feature-section  two-col">
						<div class="col col-1" style="flex: 1 1 50%;width: 100%;">
							<?php  echo
							'<h4>' .wpbc_recheck_strong_symbols( 'New features and improvements' ) . '</h4>' .
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**New**. Disable show booked times in tooltip, when mouse over a specific day in the calendar. Activate it at the Booking > Settings General page in the "Time Slots" section.' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Improvement**. Update showing total  booking cost instead of deposit,  while editing existing booking and previously  was used [cost_correction] shortcode. (9.4.4.2) *(Business Small/Medium/Large, MultiUser)*' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Improvement**. Ability to disable scrolling to calendar (open page at top of this page), after clicking on "Book now" button in search results, during searching availability. (9.4.4.6) *(Business Large, MultiUser)*' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Improvement**. Rename payment system  Ideal via "Sisow" to Ideal via "Buckaroo (former Sisow)",  because Buckaroo acquires payment service provider Sisow. *(Business Small/Medium/Large, MultiUser)*' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Improvement**. Prevent keyboard activating when using the date picker on android devices in search form fields for selection  check in/out dates (9.4.4.8) *(Business Large, MultiUser)*' ) . '</li>'
		//. '<li>' . wpbc_recheck_strong_symbols( '' ) . '</li>'
													. '</ul>' ;
							?>
						<?php  echo
							'<h4>' .wpbc_recheck_strong_symbols( 'Translations' ) . '</h4>' .
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**Translation** Turkish  [99% completed] by Basar Okke' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Translation** Brazilian Portuguese [98% Completed] by Myres Hopkins' ) . '</li>'
													. '</ul>'; ?>
						...
						<?php
							...
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( 'If you made the calendar skin customization, you need to add a new CSS section in your calendar skin. This section  starts with **FixIn: 9.5.0.2** and ends with **FixIn End: 9.5.0.2** lines in new calendar skins. Add this section  before this code line: **Dates Cells**' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'Load WP Util, that supports wp.template,  based on underscore _.template system, at front-end side. (9.4.4.11)' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'New JS event generated after inline calendar loaded: \'wpbc_datepick_inline_calendar_loaded\'. To catch this event use code: jQuery( \'body\' ).on( \'wpbc_datepick_inline_calendar_loaded\', function( event, resource_id, jCalContainer, instObj ) { ... } ); (9.4.4.12)' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'Remove some old function of support IE 7' ) . '</li>'
						...
						<?php  echo
							...
							'<ul style="list-style: disc outside;padding: 20px;margin:0;">'
		. '<li>' . wpbc_recheck_strong_symbols( '**Support**. WordPress 6.2' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Fix**. Correctly showing currency symbols in the "notes/remark" section under bookings in Booking Listing page (9.4.4.1) *(Personal, Business Small/Medium/Large, MultiUser)*' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Fix**. Correctly show "booking data" for old bookings at the Booking Listing page that was made with a custom booking form,  which  was defined as "default" custom  booking form at the Booking > Resources page. (9.4.4.9) *(Business Medium/Large, MultiUser)*' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( '**Fix**. Correctly show "booking data" at the Payment summary page, if has been used custom booking form(9.4.4.10) *(Business Medium/Large, MultiUser)*' ) . '</li>'
		. '<li>' . wpbc_recheck_strong_symbols( 'Lots of other improvements and fixes...' ) . '</li>'
													. '</ul>'; ?>
						</div>
					</div><?php
